...
class SET_instrument extends CI_Model {
    ...
    public function getFields($table) {
        $sql = $this->db->query("
            SHOW COLUMNS
            FROM $table
            WHERE Field NOT IN ('response_id', 'student_id', 'oset_class_id')
        ");
        return $sql->result();
    }
    ...
    public function setAsDefault($set_id) {
        $sql = $this->db->query("
            UPDATE set_instrument
            SET set_as_default = '0'
        ");
        $sql = $this->db->query("
            UPDATE set_instrument
            SET set_as_default = '1'
            WHERE set_instrument_id = '$set_id'
        ");
    }
